...
float sampleOpacity(vec4 albedoSample, vec2 uv) {
    if (drawPush.opacitySource == OPACITY_ONE) {
        return 1.0;
    }
...
vec3 shadedApexColor(vec3 geometricNormal, vec3 worldPos, vec2 uv, vec4 albedoSample) {
    vec3 baseColor = albedoSample.rgb;
    vec3 normal = sampleNormal(geometricNormal, worldPos, uv);
    ...
    if (camera.apexFlags.w > 0.5) {
        ...
        mat3 tbn = cotangentFrame(normal, worldPos, uv);
...
void main() {
    ...
    float opacity = clamp(sampleOpacity(albedoSample, uv), 0.0, 1.0);
    if (opacity <= 0.005) {
        discard;
    }

    vec3 color = shadedApexColor(geometricNormal, inWorldPos, uv, albedoSample);
    if (drawPush.alphaMode == ALPHA_ADDITIVE) {
        outAccum = vec4(0.0);
        outReveal = vec4(0.0);
        outAdditive = vec4(color * opacity, opacity);
        return;
    }

    float weight = clamp(opacity * 8.0 + 0.01, 0.01, 1.0);
